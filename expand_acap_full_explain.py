...
<head>
    ...
    <title>CamAI Enterprise — System Architecture, AI Models, 7 Modules & ACAP Native Edge</title>
...
<body>
    ...
    <nav class="navbar">
        ...
        <div class="nav-links">
            ...
            <a href="#acap-edge" class="nav-btn primary">ACAP Native Edge Full</a>
    ...
    <main class="container">
        ...
        <header class="hero">
            ...
            <p class="hero-desc">
                Complete A-to-Z breakdown of CamAI AI Models, 7 Specialized Analytics Modules, Team RBAC Management, Web SaaS Portal, Desktop Client, Mobile App, and ACAP Native Edge C++ Application.
            </p>
            <div class="hero-actions">
                <a href="#acap-edge" class="nav-btn primary">Explore ACAP/ Full Breakdown</a>
                <a href="#tree" class="nav-btn">Architecture Tree</a>
        ...
        <!-- SECTION: ACAP Native Edge Application (FULL END-TO-END BREAKDOWN) -->
        <section id="acap-edge" class="section-block acap">
            <div class="pillar-header">
                <div class="pillar-title-group">
                    ...
                    <div>
                        <h2 class="pillar-title">ACAP Native Edge Application (`ACAP/` Directory)</h2>
                        <div class="pillar-path">C++ Native Codebase, Hardware SDK APIs, Packaging & Testing Suite</div>
                    </div>
                </div>
            </div>
            ...
            <div class="section-grid">
                <div>
                    <p class="branch-desc">
                        `ACAP/` is a production-ready C++20 Native Edge Application compiled specifically for IP camera hardware platforms (ARTPEC-8 / ARTPEC-7). It executes 100% on-device AI analytics with sub-15ms latency, zero cloud dependency, zero CPU video copying, and full ONVIF event compliance.
                    </p>
                    <ul class="feature-list">
                        <li class="feature-item">
                            ...
                            <div><strong>VDO Stream API (`vdo_pipeline.cpp`):</strong> Zero-copy NV12 frame buffer acquisition directly from camera ISP hardware via `vdo-stream.h`.</div>
                        </li>
                        <li class="feature-item">
                            <svg class="check-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                            <div><strong>Larod NPU API (`inference.cpp`):</strong> Hardware NPU tensor acceleration for YOLOX-Tiny ONNX 8-bit models via `larod.h`.</div>
                        </li>
                        <li class="feature-item">
                            <svg class="check-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                            <div><strong>C++ ByteTrack Tracker (`tracking.cpp`):</strong> Kalman Filter motion state vectors & Hungarian matching for zero ID switches.</div>
                        </li>
                        <li class="feature-item">
                            <svg class="check-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                            <div><strong>axevent ONVIF Publisher (`events.cpp`):</strong> Native XML event producer publishing intrusion alarms to Milestone/Genetec VMS.</div>
                        </li>
                        <li class="feature-item">
                            <svg class="check-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                            <div><strong>axoverlay Bounding Box Renderer (`overlay.cpp`):</strong> Draws real-time hardware bounding boxes directly on camera H.264/H.265 streams.</div>
                        </li>
                    </ul>
                </div>
                <div>
...
void CamAIEngine::process_loop() {
    while (is_running_) {
        ...
        // 1. Larod NPU AI Inference (8-bit YOLOX)
        ...
        overlay_manager_.render(tracked, alerts);
            ...
            <!-- ACAP File Hierarchy & Module Breakdown -->
            <div class="gallery-header">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V7M3 7l9 6 9-6"/></svg>
                Complete `ACAP/` File Architecture & Component Matrix
            </div>

            <div class="grid-3">
                <div class="grid-card">
                    <span class="card-badge" style="background:#fef3c7; color:#d97706;">ACAP/manifest.json</span>
                    <div class="card-title">Manifest Schema v2.0</div>
                    <div class="card-desc">Defines app ID `camai_acap`, version `1.0.0`, `aarch64` target architecture, embedded web server endpoint, and hardware permissions.</div>
                </div>

                <div class="grid-card">
                    <span class="card-badge" style="background:#fef3c7; color:#d97706;">ACAP/Dockerfile & Makefile</span>
                    <div class="card-title">SDK Build Environment</div>
                    <div class="card-desc">Containerized ACAP Native SDK cross-compilation toolchain targeting `arm64v8/gcc` with zero host dependency requirements.</div>
                </div>

                <div class="grid-card">
                    <span class="card-badge" style="background:#fef3c7; color:#d97706;">ACAP/app/config.cpp</span>
                    <div class="card-title">axparameter Config API</div>
                    <div class="card-desc">VAPIX HTTP parameter manager for dynamic ROI threshold tuning, sensitivity adjustment, and zone persistence without app restart.</div>
                </div>

                <div class="grid-card">
                    <span class="card-badge" style="background:#fef3c7; color:#d97706;">ACAP/app/diagnostics.cpp</span>
                    <div class="card-title">ResourceGovernor Watchdog</div>
                    <div class="card-desc">Real-time memory and CPU resource governor enforcing `DROP_OLDEST` frame queue policy when RAM usage exceeds 85%.</div>
                </div>

                <div class="grid-card">
                    <span class="card-badge" style="background:#fef3c7; color:#d97706;">ACAP/build_eap_package.py</span>
                    <div class="card-title">EAP Package Builder</div>
                    <div class="card-desc">Automated packaging script that bundles manifest, model weights, C++ binary, and assets into `camai_acap_1_0_0_aarch64.eap`.</div>
                </div>

                <div class="grid-card">
                    <span class="card-badge" style="background:#fef3c7; color:#d97706;">ACAP/real_usb_phone_live_acap.py</span>
                    <div class="card-title">Real USB/IP Streamer</div>
                    <div class="card-desc">Live camera connector streaming real-time USB or IP phone camera feed to AI detector running locally at `http://localhost:8080`.</div>
                </div>
            </div>
        </section>
