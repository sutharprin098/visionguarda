...
export default function AdminStudio({
  ...
  const [mobileTab, setMobileTab] = useState<"canvas" | "cameras" | "features">("canvas");
  ...
  return (
    <div className="flex h-screen flex-col md:flex-row bg-surface-0 overflow-hidden font-sans">
      {/* MOBILE TOP BAR (Visible only on < md) */}
      <header className="flex md:hidden items-center justify-between border-b border-line bg-surface-1 px-3 py-2 shrink-0">
        <button
          onClick={onDeactivated}
          className="flex items-center gap-1 text-xs text-zinc-400 hover:text-zinc-100 font-medium"
        >
          <span>← Exit</span>
        </button>

        {/* Quick Camera Selector for Mobile */}
        <select
          ...
          className="text-xs font-semibold bg-surface-2 border border-line rounded px-2 py-1 text-zinc-100 focus:outline-none max-w-[150px] truncate"
        >
          {cameras.map((c) => (
        ...
        <button
          ...
          disabled={publishing}
          className="btn-accent px-2.5 py-1 text-[11px] font-semibold rounded flex items-center gap-1"
        >
          <Send size={11} /> {publishing ? "…" : "Publish"}
        </button>
      </header>

      {/* MOBILE SEGMENTED TAB BAR (Visible only on < md) */}
      <nav className="flex md:hidden items-center justify-around border-b border-line bg-surface-2/60 px-1 py-1 shrink-0">
        <button
          onClick={() => setMobileTab("cameras")}
          className={clsx(
            "flex-1 py-1.5 text-center text-xs font-semibold rounded transition",
            mobileTab === "cameras" ? "bg-accent/20 text-accent" : "text-zinc-400"
          )}
        >
        ...
        <button
          ...
          className={clsx(
            "flex-1 py-1.5 text-center text-xs font-semibold rounded transition",
            mobileTab === "canvas" ? "bg-accent/20 text-accent" : "text-zinc-400"
          )}
        >
          🎨 Draw Canvas
        </button>
        <button
          ...
          className={clsx(
            "flex-1 py-1.5 text-center text-xs font-semibold rounded transition",
            mobileTab === "features" ? "bg-accent/20 text-accent" : "text-zinc-400"
          )}
        >
          ⚙️ AI Rules
        </button>
      ...
      <aside
        className={clsx(
          "w-full md:w-64 border-r border-line bg-surface-1 flex flex-col justify-between shrink-0 overflow-y-auto",
          mobileTab === "cameras" ? "flex flex-1" : "hidden md:flex"
        )}
      >
          ...
          <div className="px-3 py-3">
            ...
            <div className="space-y-1">
              {cameras.map((c) => {
                ...
                return (
                  <button key={c.id} onClick={() => { setSelectedCam(c); setMobileTab("canvas"); }}
      ...
      <main
        className={clsx(
          "flex-1 flex flex-col bg-surface-0 overflow-hidden relative",
          mobileTab === "canvas" ? "flex" : "hidden md:flex"
        )}
      >
        {/* Profile selector */}
        <div className="border-b border-line bg-surface-1 px-3 py-2 shrink-0 overflow-x-auto no-scrollbar">
            ...
            {PROFILE_ORDER.map((key) => {
              ...
              return (
                <button key={key} onClick={() => selectProfile(key)} disabled={!selectedCam}
                  className={clsx("flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-medium border transition shrink-0",
        ...
        {/* Draw toolbar */}
        <div className="h-12 border-b border-line bg-surface-1 px-3 flex items-center justify-between shrink-0 overflow-x-auto no-scrollbar">
          <div className="flex items-center gap-2 min-w-max">
            ...
            <div className="flex rounded-md bg-surface-2 p-0.5 border border-line">
              {drawTools.map(({ mode, icon: Icon, label }) => (
                <button key={mode}
                  ...
                  className={clsx("text-xs px-2 py-1 rounded flex items-center gap-1 transition",
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {drawBinding && drawMode !== "view" && (
              <span className="text-[10px] text-zinc-400 hidden sm:inline">Binding: <b className="text-zinc-200">{drawBinding.featureLabel}</b></span>
            )}
        ...
        {/* Viewport */}
        <div className="flex-1 relative bg-surface-0 flex items-center justify-center p-2 md:p-4">
          {selectedCam ? (
            !activeProfile ? (
      ...
      <aside
        className={clsx(
          "w-full md:w-96 border-l border-line bg-surface-1 flex flex-col shrink-0 overflow-y-auto",
          mobileTab === "features" ? "flex flex-1" : "hidden md:flex"
        )}
      >
        {profileDef ? (
          <>
          ...
          <div className="bg-surface-1 border border-line rounded-2xl max-w-sm w-full p-5 shadow-2xl space-y-4 animate-in fade-in zoom-in duration-150">
            <div className="flex items-center justify-between border-b border-line pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white">NVR & Recording Config</h3>
                  <p className="text-[10px] text-zinc-400">Continuous clip duration & AI overlays</p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-3.5">
              ...
              <div className="p-3 rounded-xl bg-surface-2 border border-line flex items-start gap-2.5">
                ...
                />
                <label htmlFor="adminBurnInDetsMobile" className="flex-1 cursor-pointer">
